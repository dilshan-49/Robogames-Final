    ...
    for box in result.boxes:
        if(int(box.cls[0])==2):
            ...
            if(target_color == detected_color):
                ...
                cv2.putText(annotated_frame, "Target found!", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
                target_found = True
...
def box_without_color(frame):
    ...
    if(max_box is not None):
        ...
        max_box_dist = max_x_center - mid_vertical_line
    ...
    return annotated_frame,isGrabed, max_box, max_box_dist


def target_without_color(frame):
    target_found=False
    target_pixel_distance=0
    can_place = False
    max_box_area=0
    ...
    if(max_box is not None):
        target_found = True
        target_pixel_distance = max_x_center-mid_line
 
        cv2.putText(annotated_frame, f"Dist: {pixel_distance:.2f}px", (int(max_x_center), int(max_y_center) - 10),
        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

               
        if(max_box_area>100000 and has_intersection(grab_area, max_box.xyxy[0])):
            print(" Place the shit")
            can_place = True
            isGrabed = False
