...
kobuki = kd.Kobuki()  # Change port if needed
...
def move_forward():
    ...
    kobuki.move(left_velocity=0.2 , right_velocity=0.2,rotate=0)  # Move forward at 0.2 m/s
...
def main():
    ...
    while True:
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
        if color == "White":
            kobuki.move(0, 0,0)  # Stop the robot
